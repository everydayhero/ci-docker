...
readonly ec2_block_device_mappings='[{"DeviceName":"/dev/xvda","Ebs":{"DeleteOnTermination":true,"VolumeSize":%s,"VolumeType":"gp2"}}]'
...
init() {
  ...
  # The user that will be writing config and restarting services.
  env-import SSH_USER "core"

  # The user the will connect to the bastion.
...
ec2-setup-ssh-bastion() {
  ...
  if [[ "$bastion_ip" ]]; then
    SSH_OPTS="$SSH_OPTS -o \"ProxyCommand=ssh $SSH_BASTION_USER@$bastion_ip nc %h %p 2> /dev/null\""
    SSH_HOST="$(ec2-ip private $instance)"
  fi
}

ec2-ssh() {
  declare desc="SSH to instance by ip"
  declare ip="${1:?}"; shift
  local cmd
  if [[ "$1" ]]; then
    cmd="set -eo pipefail; $@"
  fi
  eval $(ssh-cmd "$SSH_USER@${ip:?}") -- "$(printf '%q' "$cmd")"
}
...
ec2-pssh() {
  ...
  ec2-list "${label:?}" \
    | jq -r .[].private_ip \
    | parallel "ec2-ssh {} $(printf '%q ' "$@")"
}

ec2-rsync() {
  declare desc="Rsync files to instance by ip"
  declare ip="${1:?}" path="$2" remote="$3"
  rsync -rzue "ssh -A ${SSH_OPTS}" "${path:?}" "$SSH_USER@${ip:?}:${remote:?}"
}
...
ec2-prsync() {
  ...
  ec2-list "${label:?}" \
    | jq -r .[].private_ip \
    | parallel "ec2-rsync {} $path $remote"
}
